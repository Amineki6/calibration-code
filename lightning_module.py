...
from scoring import compute_group_fairness_score, compute_saab_robust_auroc, compute_worst_group_accuracy
...
class CXPLightningModule(pl.LightningModule):
    def __init__(self, model: CXP_Model, method, config: ExperimentConfig):
        ...
        # EMA Model
        self.ema_model = AveragedModel(self.model, multi_avg_fn=get_ema_multi_avg_fn(config.ema_decay), use_buffers=True)
    ...
    def setup(self, stage: str):   
        ...
        if stage == 'fit':
                train_size = self._get_dataset_size(getattr(datamodule, 'train_dataset', None))
        ...
        if stage == 'test':
                test_aligned_size = self._get_dataset_size(getattr(datamodule, 'test_dataset_aligned', None))
                test_misaligned_size = self._get_dataset_size(getattr(datamodule, 'test_dataset_misaligned', None))
        
                self.test_method_aligned = self.method_template.clone(dataset_size=test_aligned_size)
                self.test_method_misaligned = self.method_template.clone(dataset_size=test_misaligned_size)
    ...
    def training_step(self, batch, batch_idx):
        # Logic from utils.py: run_training_phase loop
        batch_data = batch

        model_output = self.model(batch_data.inputs)
        extra_info = batch_data.extra_info()
        
        # Ensure train_method is initialized
        ...
        self.log("train/brier", brier, on_step=False, on_epoch=True, metric_attribute="train_brier")
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        with torch.no_grad():
            model_output = self.model(batch_data.inputs)
            self.train_method.update_loss(model_output, batch_data.labels, 
                                          extra_info=extra_info,
                                          weight=batch_data.usable_weights)

    def on_train_epoch_end(self):
        # Update EMA Model
        # utils.py: ema_model_compiled.update_parameters(model_compiled)
        # This updates the Moving Average of the parameters. 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Use validation-specific method if available (cloned in setup), else use main template
    ...
    def on_validation_epoch_end(self):
        ...
        if 'summary' in fairness_details:
            self.log("val/fairness_mean_pairwise_auroc", fairness_details['summary']['mean_pairwise_auroc'], on_epoch=True)
            self.log("val/fairness_min_pairwise_auroc", fairness_details['summary']['min_pairwise_auroc'], on_epoch=True)
            self.log("val/fairness_arithmetic_mean", fairness_details['summary'].get('arithmetic_mean', 0.0), on_epoch=True)
            self.log("val/fairness_geometric_mean", fairness_details['summary'].get('geometric_mean', 0.0), on_epoch=True)
            self.log("val/fairness_harmonic_mean", fairness_details['summary'].get('harmonic_mean', 0.0), on_epoch=True)

        if 'pairwise' in fairness_details:
            for pair_key, pair_info in fairness_details['pairwise'].items():
                if 'auroc' in pair_info:
                    self.log(f"val/pairwise_auroc_{pair_key}", pair_info['auroc'], on_epoch=True)
        ...
        worst_acc, _ = compute_worst_group_accuracy(all_logits, all_labels, intersectional_groups)
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if dataloader_idx == 0:
            # Aligned
            method = self.test_method_aligned if self.test_method_aligned else self.method_template
            ...
            # Misaligned
            method = self.test_method_misaligned if self.test_method_misaligned else self.method_template
            loss, _ = method.compute_loss(
                (logits, projections),
                batch_data.labels,
                weight=batch_data.usable_weights,
            ...
            self.log("test/loss_misaligned", loss, on_epoch=True, add_dataloader_idx=False)
    ...
    def on_test_epoch_end(self):
        ...
        # Saab robust AUROC is computed on the merged test set using:
        ...
        if merged_test_outputs:
            ...
            if 'summary' in fairness_details:
                self.log("test/fairness_mean_pairwise_auroc", fairness_details['summary']['mean_pairwise_auroc'])
                self.log("test/fairness_min_pairwise_auroc", fairness_details['summary']['min_pairwise_auroc'])
                self.log("test/fairness_arithmetic_mean", fairness_details['summary'].get('arithmetic_mean', 0.0))
                self.log("test/fairness_geometric_mean", fairness_details['summary'].get('geometric_mean', 0.0))
                self.log("test/fairness_harmonic_mean", fairness_details['summary'].get('harmonic_mean', 0.0))
            ...
            if robust_auroc is not None:
                ...
                logging.info("test/robust_auroc_saab: nan (insufficient robust subset diversity)")
            ...
            if valid_group_mask.any():
                ...
                for label_value in unique_labels:
                    for drain_value in unique_groups:
                        subgroup_mask = (
                            (merged_labels_valid == label_value)
                            & (merged_drains_valid == drain_value)
                        )
                        if subgroup_mask.any():
                            metric_name = f"test/mean_score_y{int(label_value)}_g{int(drain_value)}"
                            self.log(metric_name, merged_probs_valid[subgroup_mask].mean())
        
        if self.test_aligned_outputs:
            labels_aligned = torch.cat([x["label"] for x in self.test_aligned_outputs]).float()
        ...
        if self.test_misaligned_outputs:
            labels_misaligned = torch.cat([x["label"] for x in self.test_misaligned_outputs]).float()
            probs_misaligned = torch.cat([x["y_prob"] for x in self.test_misaligned_outputs])
            
            brier_misaligned = ((probs_misaligned - labels_misaligned) ** 2).mean()
            calib_dist_misaligned = torch.abs(probs_misaligned.mean() - labels_misaligned.mean())
    ...
    def configure_optimizers(self):
        ...
        if self.model.projection_head is not None:
             params.append({'params': self.model.projection_head.parameters(), 'lr': self.config.lr})

        if torch.cuda.is_available() and torch.__version__ >= '2.0':
            optimizer = optim.AdamW(params, weight_decay=self.config.weight_decay, eps=1e-10, fused=True)
        else:
            optimizer = optim.AdamW(params, weight_decay=self.config.weight_decay, eps=1e-10)
