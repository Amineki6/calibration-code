...
class MMDLoss(nn.Module):
    ...
    def gaussian_kernel(self, source: torch.Tensor, target: torch.Tensor):
        ...
        if fix_sigma:
            ...
            bandwidth = torch.sum(L2_distance_squared.detach()) / (
        ...
        kernel_val = [
            torch.exp(-L2_distance_squared / bandwidth_temp)
...
class MMDMethod(BaseMethod):
    ...
    def compute_loss(
        ...
        bce_loss, wbce_loss = self.compute_bce_terms(
            logits, labels, weight=sample_weights
        )
        cls_loss = bce_loss
